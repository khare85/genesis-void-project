...
const SkillsSection: React.FC<SkillsSectionProps> = ({
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <input id="level" name="level" type="range" min="1" max="100" value={level} onChange={e => setLevel(parseInt(e.target.value))} className="flex-1" />
  ...
  return <Card>
      <CardHeader className="py-4 bg-transparent">
      ...
      <CardContent className="pt-0 rounded-2xl bg-transparent">
          ...
          {skills.length > 0 ? skills.map((skill: any, index: number) => <div key={skill.id || index} className="space-y-2">
                {isEditing ? <div className="grid gap-2">
                    <div className="flex gap-2">
                      <FormField control={form?.control} name={`skills.${index}.name`} render={({
                field
                      ...
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteSkill(index)} className="h-10 w-10 text-destructive hover:text-destructive hover:bg-destructive/10">
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                    <FormField control={form?.control} name={`skills.${index}.level`} render={({
              field
            }) => <FormItem>
                          <div className="flex justify-between items-center">
                          ...
                          <FormControl>
                            <input type="range" min={1} max={100} className="w-full" {...field} onChange={e => field.onChange(parseInt(e.target.value))} />
                          </FormControl>
                        </FormItem>} />
                    ...
                    <div className="h-2 w-full bg-gray-100 rounded">
                      <div style={{
                width: `${skill.level}%`
              }} className="h-full rounded bg-orange-600"></div>
                    </div>
                  </div>}
              </div>) : <div className="text-center py-4">
              <p className="text-sm text-muted-foreground">No skills added yet</p>
              {isEditing && <AddItemModal title="Add Skill" description="Add a new skill to your profile" triggerText="Add a skill" buttonVariant="ghost" buttonSize="sm" className="mt-2">
                  {({
              close
            }) => <SkillForm onSubmit={data => {
              if (handleAddSkill(data)) close();
