        ...
        {certificates && certificates.length > 0 ? certificates.map((certificate: any, index: number) => <div key={certificate.id || index} className="border rounded-lg p-6 bg-white shadow-sm">
              ...
              field
                  ...
                  <div>
                    ...
                    {certificate.credentialId && <p className="text-sm mt-1">Credential ID: {certificate.credentialId}</p>}
                  </div>
                </div>}
            ...
            {isEditing && <AddItemModal title="Add Certificate" description="Add details about your professional certifications" triggerText="Add Certificate" buttonVariant="ghost" className="mt-2 text-primary hover:text-primary-dark">
                {({
            close
          }) => <CertificateForm onSubmit={data => {
            if (handleAddCertificate(data)) close();
