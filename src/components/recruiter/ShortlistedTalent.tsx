...
export const ShortlistedTalent = () => {
  ...
  useEffect(() => {
    if (screeningData && screeningData.length > 0) {
      ...
      const shortlisted = screeningData.filter(candidate => candidate.status === 'shortlisted' || candidate.status === 'interview');
      ...
      const jobsArray = Object.values(jobGroups).sort((a, b) => b.count - a.count).slice(0, 3);
      setShortlistedJobs(jobsArray);
    }
  }, [screeningData]);
  return <Card className="bg-white shadow-lg transform transition-all hover:shadow-xl hover:-translate-y-1 border-0 rounded-2xl">
      <div className="p-6">
