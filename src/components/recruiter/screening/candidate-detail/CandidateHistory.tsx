...
export const CandidateHistory: React.FC<CandidateHistoryProps> = ({ candidateId }) => {
  const { pastApplications, isLoading } = usePastApplications(candidateId);
  ...
  return (
    <Card className="border-0 shadow-none">
      ...
      <CardContent className="px-0 py-0 space-y-2">
        <div className="space-y-2">
          {pastApplications.map((app, index) => (
            <div key={index} className="flex items-center justify-between py-2 border-b last:border-b-0 text-sm">
