...
export const SkillsInput: React.FC<SkillsInputProps> = ({
  ...
  return <div className="space-y-2">
      <Input placeholder="Type a skill and press Enter" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyDown={handleKeyDown} onBlur={() => {
      if (inputValue) {
        addSkill(inputValue);
      }
    }} />
      <div className="flex flex-wrap gap-2">
        {skills.map((skill, index) => <div key={index} className="flex items-center gap-1 bg-primary bg-opacity-10 text-primary px-2 py-1 rounded-md">
            <Tag className="h-3 w-3" />
            <span className="text-sm text-white">{skill}</span>
            <button type="button" onClick={() => removeSkill(skill)} className="text-primary text-opacity-50 hover:text-primary">
              <X className="h-3 w-3" />
            </button>
          </div>)}
      </div>
